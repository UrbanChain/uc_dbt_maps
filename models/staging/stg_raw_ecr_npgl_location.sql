-- models/staging/stg_raw_ecr_npgl_location.sql

with stg_raw_ecr_npgl_1_location as (
    select
        export_mpan,
        import_mpan,
        customer_name,
    ...
    from {{ ref('stg_raw_ecr_npgl_1') }}
),

stg_raw_ecr_npgl_2_location as (
    select
        export_mpan,
        import_mpan,
        customer_name,
        ...
        -- Create geopoint in EPSG:27700 (British National Grid)
        st_setsrid(st_makepoint(x_coordinate::float, y_coordinate::float), 27700) as geopoint
    from {{ ref('stg_raw_ecr_npgl_2') }}
),

-- Combine both sources into one table
combined_location as (
    select * from stg_raw_ecr_npgl_1_location
    union all
    select * from stg_raw_ecr_npgl_2_location
)
