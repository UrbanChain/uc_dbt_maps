with source as (
    select * from {{ source('raw_ecr_npgl_2_source', 'raw_ecr_npgl_2') }}
)
...
select
    ...
    -- Integer columns with direct casting
    ...
    case
        when LOWER("Point of Connection (POC) Voltage (kV)"::text) IN ('', 'data not available')
        then NULL
        else "Point of Connection (POC) Voltage (kV)"::real
    end as poc_voltage,
    ...
    -- Storage capacity and duration columns with case-insensitive handling for real conversion
    case
    ...
    -- Real column with direct casting for registered capacity
    "Energy Source & Energy Conversion Technology 1 - Registered Cap"::real as registered_cap_1,
    ...
    case
        when LOWER({{ clean_value('"Energy Source & Energy Conversion Technology 2 - Registered Cap"') }}) IN ('', 'data not available')
        then NULL
        else {{ clean_value('"Energy Source & Energy Conversion Technology 2 - Registered Cap"') }}::real
    end as energy_conversion_tech_2_cap,
    ...
    case
        when LOWER({{ clean_value('"Energy Source & Energy Conversion Technology 3 - Registered Cap"') }}) IN ('', 'data not available')
        then NULL
        else {{ clean_value('"Energy Source & Energy Conversion Technology 3 - Registered Cap"') }}::real
    end as energy_conversion_tech_3_cap,

    {{ clean_value('"Flexible Connection (Yes/No)"') }} as flexible_connection,
    {{ clean_value('"Connection Status"') }} as connection_status,

    "Already connected Registered Capacity (MW)"::real as already_connected_capacity_mw,

    case
        when LOWER("Maximum Export Capacity (MW)"::text) IN ('', 'data not available')
        then NULL
        else "Maximum Export Capacity (MW)"::real
    end as max_export_capacity_mw,

    case
        when LOWER("Maximum Export Capacity (MVA)"::text) IN ('', 'data not available')
        then NULL
        else "Maximum Export Capacity (MVA)"::real
    end as max_export_capacity_mva,

    case
        when LOWER("Maximum Import Capacity (MW)"::text) IN ('', 'data not available')
        then NULL
        else "Maximum Import Capacity (MW)"::real
    end as max_import_capacity_mw,

    case
        when LOWER({{ clean_value('"Maximum Import Capacity (MVA)"') }}) IN ('', 'data not available')
        then NULL
        else {{ clean_value('"Maximum Import Capacity (MVA)"') }}::real
    end as max_import_capacity_mva,

    {{ convert_to_datetime(clean_value('"Date Connected"')) }} as date_connected,

    "Accepted to Connect Registered Capacity (MW)"::integer as accepted_to_connect_capacity_mw,

    case
        when LOWER("Change to Maximum Export Capacity (MW)"::text) IN ('', 'data not available')
        then NULL
        else "Change to Maximum Export Capacity (MW)"::real
    end as change_to_max_export_capacity_mw,

    case
        when LOWER("Change to Maximum Export Capacity (MVA)"::text) IN ('', 'data not available')
        then NULL
        else "Change to Maximum Export Capacity (MVA)"::real
    end as change_to_max_export_capacity_mva,

    case
        when LOWER("Change to Maximum Import Capacity (MW)"::text) IN ('', 'data not available')
        then NULL
        else "Change to Maximum Import Capacity (MW)"::real
    end as change_to_max_import_capacity_mw,

    case
        when LOWER("Change to Maximum Import Capacity (MVA)"::text) IN ('', 'data not available')
        then NULL
        else "Change to Maximum Import Capacity (MVA)"::real
    end as change_to_max_import_capacity_mva,
